...
exports.config = {
    ...
    services: [[TimelineService, { drivers:{chrome:"123.0.6312.105"}}]],
    //
    ...
    reporters: [['timeline', { outputDir: './report' }]],
    ...
    before: function (capabilities, specs) {
        browser.addCommand('open_page', async (page) => {
            await Helper.open_page(page)
        })
        browser.addCommand('login', async (username, password, result) => {
            await Helper.type_info('#username', username)
            await Helper.type_info('#password', password)
            await Helper.click('button[type="submit"]')
            await Helper.assert_paragraph('#flash', result)
        })
